...
public class PathingMain extends PApplet
{
   ...
   private boolean moveOnce(Point pos, GridValues[][] grid, List<Point> path) {
      ...
      if (withinBounds(pos, grid) && grid[pos.y][pos.x] != GridValues.OBSTACLE && grid[pos.y][pos.x] != GridValues.SEARCHED) {
         ...
         Point leftPoint = new Point(pos.x - 1, pos.y);
         Point rightPoint = new Point(pos.x + 1, pos.y);
         Point abovePoint = new Point(pos.x, pos.y - 1);
         Point belowPoint = new Point(pos.x, pos.y + 1);

         if (moveOnce(rightPoint, grid, path) ||
               moveOnce(leftPoint, grid, path) ||
               moveOnce(belowPoint, grid, path) ||
               moveOnce(abovePoint, grid, path)) {
            path.add(0, pos);
            return true;
         }
      }
...
//         grid[rightN.y][rightN.x] = GridValues.SEARCHED;
//         grid[rightN.y + 1][rightN.x] = GridValues.SEARCHED;
//         path.add(0, rightN);
//      }
//      return false;
//   }
